...
class CompanyEnricher:
    """Анализирует текстовые данные о компаниях для извлечения бизнес-признаков.

    Использует spaCy с русской моделью для NLP-обработки текстов (например, с веб-сайтов)
    и определяет:
    ...
    def __init__(self, min_text_length=20):
        """Инициализирует обогатитель, загружая spaCy-модель и настраивая ключевые слова.

        Args:
            min_text_length (int): Минимальная длина текста для запуска анализа.
                                   Короткие тексты пропускаются. По умолчанию — 20.
    ...
    def _detect_importer(self, doc):
        """Определяет, является ли компания импортёром, на основе анализа текста.
    ...
    def _extract(self, doc, keys):
        """Извлекает уникальные леммы из текста, совпадающие с заданным списком.
    ...
    def _find_indicators(self, doc):
        ...
        Returns:
            int: Количество найденных индикаторов (например, «контракт», «продажи»).
        """
    ...
    def _detect_recent_activity(self, doc):
        ...
        Returns:
            bool: True, если в тексте есть указания на активность в 2024–2025 гг.
        """
    ...
    def _empty(self, company_id):
        """Создаёт заглушку признаков для компаний с недостаточным или пустым текстом.

        Args:
            company_id (any): Идентификатор компании.

        Returns:
            dict: Словарь признаков со значениями по умолчанию (всё False/0/[]).
        """
        return {
